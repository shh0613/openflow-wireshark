...
void FieldManager::createField(std::string key, const char *name, ftenum type, int display,
                    const void *strings, guint32 bitmask, bool subtree)
{
...
{
    proto_register_field_array(this->mProto, (hf_register_info*) this->mFieldArray->data,
        this->mFieldArray->len);
    proto_register_subtree_array((gint* const*)this->mTreeArray->data, this->mTreeArray->len);
}

proto_tree* FieldManager::addSubtree(proto_tree *tree, std::string key, tvbuff_t *tvb,
                            guint32 start, guint32 len)
{
...
void FieldManager::addItem(proto_tree *tree, std::string key, tvbuff_t *tvb, guint32 start,
                    guint32 len)
{
...
void FieldManager::addString(proto_tree *tree, std::string key, tvbuff_t *tvb, guint32 start,
                    guint32 len, const char *str)
{
    // TODO: move this check to a function
    if (this->mFields.find(key) != this->mFields.end()) {
        proto_tree_add_string(tree, *(this->mFields[key]), tvb, start, len, str);
    } else {
        std::cerr << "Couldn't find key: " << key << std::endl;
    }
}

proto_item* FieldManager::addBoolean(proto_tree *tree, std::string key, tvbuff_t *tvb,
                            guint32 start, guint32 len, guint32 value)
{
    // TODO: move this check to a function
    if (this->mFields.find(key) != this->mFields.end()) {
...
proto_item* FieldManager::addUint(proto_tree *tree, std::string key, tvbuff_t *tvb, guint32 start,
                            guint32 len, guint32 value)
{
    // TODO: move this check to a function
    if (this->mFields.find(key) != this->mFields.end()) {
        return proto_tree_add_uint(tree, *(this->mFields[key]), tvb, start, len, value);
    } else {
        std::cerr << "Couldn't find key: " << key << std::endl;
    }
...
void FieldManager::addDissector (proto_tree *tree, std::string key, tvbuff_t *tvb,
                    packet_info *pinfo, dissector_handle_t handle, guint32 start, guint32 len)
{
    // TODO: move this check to a function
    if (this->mFields.find(key) != this->mFields.end()) {
    	tvbuff_t *next_tvb;
        
        /* add seperators to existing column strings by shh0613 */
        ...
        /* set up fences so ethernet dissectors only appends to our column info by shh0613 */
        col_set_fence(pinfo->cinfo, COL_PROTOCOL);
        col_set_fence(pinfo->cinfo, COL_INFO);

    	/* Create the tvbuffer for the next dissector by shh0613 */
